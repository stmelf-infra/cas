 ...
 * <li>Determines the next webflow state by comparing handler erors against {@link #errors} in list order. The first
...
public class AuthenticationExceptionHandler {
	 ...
	 * {@link org.jasig.cas.authentication.AuthenticationException#getHandlerErrors()} with highest precedence. Also
	 * sets an ERROR severity message in the message context of the form
	 * <code>[messageBundlePrefix][exceptionClassSimpleName]</code> for each handler error that is configured. If not
	 * match is found, {@value #UNKNOWN} is returned.
	 *
	 * @param e
	 *            Authentication error to handle.
	 *
	...
	public String handle(final AuthenticationException e, final MessageContext messageContext) {
		...
		logger.trace(
				"Unable to translate handler errors of the authentication exception {}. Returning {} by default...",
				e,
				messageCode);
		messageContext.addMessage(new MessageBuilder().error().code(messageCode).build());
		return UNKNOWN;
