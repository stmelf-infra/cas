 ...
 *
 * AbstractTicket does not provide a protected Logger instance to avoid instantiating many such Loggers at runtime
 * (there will be many instances of subclasses of AbstractTicket in a typical running CAS server). Instead subclasses
 * should use static Logger instances.
 *
...
public abstract class AbstractTicket implements Ticket, TicketState {
	...
	/**
	 * Records the <i>previous</i> last time this ticket was used as well as the last usage time. The ticket usage count
	 * is also incremented.
	 *
	 * <p>
	 * Tickets themselves are solely responsible to maintain their state. The determination of ticket usage is left up
	 * to the implementation and the specific ticket type.
	 *
