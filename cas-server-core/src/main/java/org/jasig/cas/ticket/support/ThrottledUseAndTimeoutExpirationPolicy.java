...
/**
 * Implementation of an expiration policy that adds the concept of saying that a ticket can only be used once every X
 * millseconds to prevent misconfigured clients from consuming resources by doing constant redirects.
 *
...
public final class ThrottledUseAndTimeoutExpirationPolicy implements ExpirationPolicy, Serializable {
	...
	public boolean isExpired(final TicketState ticketState) {
		if (ticketState.getCountOfUses() == 0
			...
			LOGGER.debug(
					"Ticket is not expired due to a count of zero and the time being less "
