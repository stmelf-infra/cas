...
public final class TicketGrantingTicketImpl extends AbstractTicket implements TicketGrantingTicket {
	...
	public TicketGrantingTicketImpl(
		...
		this.userId = getUserId(authentication);
	}
	 ...
	 * {@inheritDoc}
	 * <p>
	 * The state of the ticket is affected by this operation and the ticket will be considered used. The state update
	 * subsequently may impact the ticket expiration policy in that, depending on the policy configuration, the ticket
	...
	public synchronized ServiceTicket grantServiceTicket(
		...
		final ServiceTicket serviceTicket = new ServiceTicketImpl(
				...
				this,
				service,
				this.getCountOfUses() == 0 || credentialsProvided,
				expirationPolicy);
