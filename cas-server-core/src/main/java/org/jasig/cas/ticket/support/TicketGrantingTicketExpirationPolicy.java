...
/**
 * Provides the Ticket Granting Ticket expiration policy. Ticket Granting Tickets can be used any number of times, have
 * a fixed lifetime, and an idle timeout.
 *
...
public final class TicketGrantingTicketExpirationPolicy implements ExpirationPolicy, InitializingBean, Serializable {
	...
	/**
	 * @deprecated As of 4.1. Set max time to live in milli seconds.
	 *
	 * @param maxTimeToLiveInMilliSeconds
	 *            the max time to live in milli seconds
	 */
	@Deprecated
	 ...
	 *            max number of seconds for the tickets to stay alive
	 **/
	 ...
	 *            time for the ticket to stay active in seconds Convenient virtual property setter to set time in
	 *            seconds.
	 **/
	...
	public boolean isExpired(final TicketState ticketState) {
		...
		if ((System.currentTimeMillis() - ticketState.getCreationTime() >= maxTimeToLiveInMilliSeconds)) {
			LOGGER.debug(
					"Ticket is expired because the time since creation is greater than maxTimeToLiveInMilliSeconds");
			return true;
		}
