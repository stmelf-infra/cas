...
public class JpaLockingStrategy implements LockingStrategy {
	...
	/**
	 * Application identifier that identifies rows in the locking table, each one of which may be for a different
	 * application or usage within a single application.
	 */
	@NotNull
	...
	/**
	 * @param id
	 *            Application identifier that identifies a row in the lock table for which multiple clients vie to hold
	 *            the lock. This must be the same for all clients contending for a particular lock.
	 */
	...
	/**
	 * @param id
	 *            Identifier used to identify this instance in a row of the lock table. Must be unique across all
	 *            clients vying for locks for a given application ID.
	 */
	...
	/**
	 * @param seconds
	 *            Maximum amount of time in seconds lock may be held. A value of zero indicates that locks are held
	 *            indefinitely. Use of a reasonable timeout facilitates recovery from node failures, so setting to zero
	 *            is discouraged.
	 */
