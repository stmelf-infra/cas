...
/**
 * Monitors the status of a {@link org.jasig.cas.ticket.registry.TicketRegistry} that supports the
 * {@link TicketRegistryState} interface for exposing internal state information used in status reports.
 *
...
public class SessionMonitor implements Monitor<SessionStatus> {
	 ...
	 * 
	 * @param state
	 *            the ticket registry state instance
	 */
	 ...
	 *
	 * @param threshold
	 *            Warn threshold if non-negative value, otherwise warnings are disabled.
	 */
		...
		try {
			...
			if (sessionCount == Integer.MIN_VALUE || ticketCount == Integer.MIN_VALUE) {
				return new SessionStatus(
						StatusCode.UNKNOWN,
						String.format(
								"Ticket registry %s reports unknown session and/or ticket counts.",
			...
			if (this.sessionCountWarnThreshold > -1 && sessionCount > this.sessionCountWarnThreshold) {
				...
				msg.append(
						String.format(
								"Session count (%s) is above threshold %s. ",
								sessionCount,
								this.sessionCountWarnThreshold));
			}
			else {
