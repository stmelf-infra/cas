...
/**
 * Describes a one-time-password credential that contains an optional unique identifier and required password. The
 * primary difference between this component and {@link UsernamePasswordCredential} is that the username/ID is optional
 ...
 * authentication. Note that metadata is stored only _after_ authentication, at which time the OTP has already been
 * consumed and by definition is no longer useful for authentication.
 *
...
public class OneTimePasswordCredential extends AbstractCredential {
	 ...
	 *
	 * @param password
	 *            Non-null cleartext one-time password value.
	 */
	 ...
	 *
	 * @param id
	 *            Identifier that is commonly used to look up one-time password in system of record.
	 * @param password
	 *            Non-null cleartext one-time password value.
	 */
