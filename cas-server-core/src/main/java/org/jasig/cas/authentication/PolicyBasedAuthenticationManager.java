...
/**
 * Provides an authenticaiton manager that is inherently aware of multiple credentials and supports pluggable security
 * policy via the {@link AuthenticationPolicy} component. The authentication process is as follows:
 *
 * <ul>
 ...
 * </ul>
 * </ul>
 * <li>After all credentials have been attempted check security policy again. Note there is an implicit security policy
 * that requires at least one credential to be authenticated. Then the security policy given by
 * {@link #setAuthenticationPolicy(AuthenticationPolicy)} is applied. In all cases {@link AuthenticationException} is
 * raised if security policy is not met.</li>
 * </ul>
 *
...
public class PolicyBasedAuthenticationManager implements AuthenticationManager {
	 ...
	 * listed order for supported credentials. This form may only be used by authentication handlers that resolve
	 * principals during the authentication process.
	 *
	 * @param handlers
	 *            One or more authentication handlers.
	 */
	...
	/**
	 * Creates a new authentication manager with a list of authentication handlers that are attempted in the listed
	 * order for supported credentials. This form may only be used by authentication handlers that resolve principals
	 * during the authentication process.
	 *
	 * @param handlers
	 *            Non-null list of authentication handlers containing at least one entry.
	 */
	...
	/**
	 * Creates a new authentication manager with a map of authentication handlers to the principal resolvers that should
	 * be used upon successful authentication if no principal is resolved by the authentication handler. If the order of
	 * evaluation of authentication handlers is important, a map that preserves insertion order (e.g.
	 * {@link LinkedHashMap}) should be used.
	 *
	 * @param map
	 *            Non-null map of authentication handler to principal resolver containing at least one entry.
	 */
	...
	public final Authentication authenticate(final Credential... credentials) throws AuthenticationException {
		...
		for (final AuthenticationMetaDataPopulator populator : this.authenticationMetaDataPopulators) {
			for (final Credential credential : credentials) {
				populator.populateAttributes(builder, credential, principal);
			}
		}
	 ...
	 *
	 * @param populators
	 *            Non-null list of metadata populators.
	 */
	 ...
	 *
	 * @param policy
	 *            Non-null authentication policy. The default policy is {@link AnyAuthenticationPolicy}.
	 */
	 ...
	 *
	 * @param credentials
	 *            One or more credentials to authenticate.
	 *
	 ...
	 *             When one or more credentials failed authentication such that security policy was not satisfied.
	 */
	...
	/**
	 * Null prinicpal implementation that allows us to construct {@link Authentication}s in the event that no principal
	 * is resolved during the authentication process.
	 */
