...
/**
 * A credential representing an HTTP endpoint given by a URL. Authenticating the credential usually involves contacting
 * the endpoint via the URL and observing the resulting connection (e.g. SSL certificate) and response (e.g. status,
 * headers).
 *
...
public class HttpBasedServiceCredential extends AbstractCredential {
	 ...
	 *
	 * @param callbackUrl
	 *            Non-null URL that will be contacted to validate the credential.
	 *
	 * @throws IllegalArgumentException
	 *             if the callbackUrl is null.
	 */
	...
	public int hashCode() {
		...
		result = prime
				* result
				+ ((this.callbackUrlAsString == null) ? 0
						: this.callbackUrlAsString
								.hashCode());
		return result;
	}
