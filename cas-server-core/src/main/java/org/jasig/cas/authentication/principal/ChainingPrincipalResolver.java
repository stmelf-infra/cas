 ...
 * <p>
 * A common use case for this component is resolving a temporary principal ID from an X.509 credential followed by a
 * search (e.g. LDAP, database) for the final principal based on the temporary ID.
 *
...
public class ChainingPrincipalResolver implements PrincipalResolver {
	...
	/**
	 * Sets the resolver chain. The resolvers other than the first one MUST be capable of performing resolution on the
	 * basis of {@link org.jasig.cas.authentication.Credential#getId()} alone; {@link PersonDirectoryPrincipalResolver}
	 * notably meets that requirement.
	 *
	 * @param chain
	 *            List of delegate resolvers that are invoked in a chain.
	 */
	 ...
	 * {@link PrincipalResolver#supports(org.jasig.cas.authentication.Credential)} method is called only for the first
	 * configured resolver.
	 *
	 * @param credential
	...
	/**
	 * Determines whether the credential is supported by this component by delegating to the first configured resolver
	 * in the chain.
	 *
	 * @param credential
	 *            The credential to check for support.
	 *
