...
/**
 * Class to validate the credential presented by communicating with the web server and checking the certificate that is
 * returned against the hostname, etc.
 * <p>
 * This class is concerned with ensuring that the protocol is HTTPS and that a response is returned. The SSL handshake
 * that occurs automatically by opening a connection does the heavy process of authenticating.
 *
...
public final class HttpBasedServiceCredentialsAuthenticationHandler extends AbstractAuthenticationHandler {
	...
	/**
	 * @return true if the credential provided are not null and the credential are a subclass of (or equal to)
