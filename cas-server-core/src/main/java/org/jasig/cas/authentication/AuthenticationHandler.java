...
public interface AuthenticationHandler {
	...
	/**
	 * Authenticates the given credential. There are three possible outcomes of this process, and implementers MUST
	 ...
	 *
	 * @return A result object containing metadata about a successful authentication event that includes at a minimum
	 *         the name of the handler that authenticated the credential and some credential metadata. The following
	 *         data is optional:
	 *         <ul>
	 ...
	 *             On authentication failures where the root cause is security related, e.g. invalid credential.
	 *             Implementing classes SHOULD be as specific as possible in communicating the reason for authentication
	 *             failure. Recommendations for common cases:
	 *             <ul>
	 ...
	 *             On errors that prevented authentication from occurring. Implementing classes SHOULD take care to
	 *             populate the cause, where applicable, with the error that prevented authentication.
	 */
	HandlerResult authenticate(Credential credential) throws GeneralSecurityException, PreventedException;

	/**
	 * Determines whether the handler has the capability to authenticate the given credential. In practical terms, the
	 * {@link #authenticate(Credential)} method MUST be capable of processing a given credential if
	...
	/**
	 * Gets a unique name for this authentication handler within the Spring context that contains it. For
	 * implementations that allow setting a unique name, deployers MUST take care to ensure that every handler instance
	 * has a unique name.
	 *
