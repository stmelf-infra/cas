...
/**
 * The regex filter that is responsible to make sure only attributes that match a certain regex pattern registered
...
public class RegisteredServiceRegexAttributeFilter implements RegisteredServiceAttributeFilter {
	 ...
	 * <li>Multi-valued attributes such as those of type {@link Collection} and {@link Map} are expected to allow
	 * casting to <code>Map&lt;String, String&gt;</code> or <code>Collection&lt;String&gt;</code>. Values that are of
	 * type array are expected to allow casting to <code>String[]</code>.</li>
	 * <li>Multi-valued attributes are always put back into the final released collection of attributes as
	 * <code>String[]</code>.</li>
	...
	public Map<String, Object> filter(
			...
			if (attributeValue != null) {
				if (attributeValue instanceof Collection) {
					final String[] filteredAttributes = filterArrayAttributes(
							((Collection<String>) attributeValue).toArray(new String[] {}),
							attributeName);
		...
		logger.debug(
				"Received {} attributes. Filtered and released {}",
				givenAttributes.size(),
	...
	private void logReleasedAttributeEntry(final String attributeName, final String attributeValue) {
		logger.debug(
				"The attribute value [{}] for attribute name {} matches the pattern {}. Releasing attribute...",
				attributeValue,
				attributeName,
