...
/**
 * Concrete implementation of a CentralAuthenticationService, and also the central, organizing component of CAS's
 ...
 * <ul>
 * <li><code>ticketRegistry</code> - The Ticket Registry to maintain the list of available tickets.</li>
 * <li><code>serviceTicketRegistry</code> - Provides an alternative to configure separate registries for TGTs and ST in
 * order to store them in different locations (i.e. long term memory or short-term)</li>
 * <li><code>authenticationManager</code> - The service that will handle authentication.</li>
 * <li><code>ticketGrantingTicketUniqueTicketIdGenerator</code> - Plug in to generate unique secure ids for
 * TicketGrantingTickets.</li>
 * <li><code>serviceTicketUniqueTicketIdGenerator</code> - Plug in to generate unique secure ids for
 * ServiceTickets.</li>
 * <li><code>ticketGrantingTicketExpirationPolicy</code> - The expiration policy for TicketGrantingTickets.</li>
 * <li><code>serviceTicketExpirationPolicy</code> - The expiration policy for ServiceTickets.</li>
 * </ul>
 *
...
public final class CentralAuthenticationServiceImpl implements CentralAuthenticationService {
	...
	/**
	 * New Ticket Registry for storing and retrieving services tickets. Can point to the same one as the ticketRegistry
	 * variable.
	 */
	@NotNull
	...
	/**
	 * {@inheritDoc} Destroy a TicketGrantingTicket and perform back channel logout. This has the effect of invalidating
	 * any Ticket that was derived from the TicketGrantingTicket being destroyed. May throw an
	...
	public List<LogoutRequest> destroyTicketGrantingTicket(final String ticketGrantingTicketId) {
		...
		final TicketGrantingTicket ticket = this.ticketRegistry.getTicket(
	 ...
	 *             if ticketGrantingTicketId or service are null.
	 */
	...
	/**
	 * @throws IllegalArgumentException
	 *             if the ServiceTicketId or the Credential are null.
	 */
		...
		try {
			synchronized (serviceTicket) {
				...
				if (!serviceTicket.isValidFor(service)) {
					logger.error(
							"ServiceTicket [{}] with service [{}] does not match supplied service [{}]",
							serviceTicketId,
							serviceTicket.getService().getId(),
							service);
			...
			Map<String, Object> attributesToRelease = this.defaultAttributeFilter.filter(
					principal.getId(),
					principal.getAttributes(),
					registeredService);
			if (registeredService.getAttributeFilter() != null) {
				attributesToRelease = registeredService.getAttributeFilter().filter(
						principal.getId(),
						attributesToRelease,
						registeredService);
			}
	 ...
	 * <li>If the username attribute is available as part of the principal's attributes, the corresponding attribute
	 ...
	 * @param serviceTicket
	 *            An instance of the service ticket used for validation
	 *
