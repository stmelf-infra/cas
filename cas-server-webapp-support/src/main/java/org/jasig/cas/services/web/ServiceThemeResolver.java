...
/**
 * ThemeResolver to determine the theme for CAS based on the service provided. The theme resolver will extract the
 * service parameter from the Request object and attempt to match the URL provided to a Service Id. If the service is
 * found, the theme associated with it will be used. If not, these is associated with the service or the service was not
 * found, a default theme will be used.
 *
...
public final class ServiceThemeResolver extends AbstractThemeResolver {
	...
	/**
	 * Sets the map of mobile browsers. This sets a flag on the request called "isMobile" and also provides the custom
	 * flag called browserType which can be mapped into the theme.
	 * <p>
