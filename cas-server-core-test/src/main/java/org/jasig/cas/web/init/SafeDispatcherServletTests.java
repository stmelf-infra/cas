...
public class SafeDispatcherServletTests {
	...
	/*
	 * Test that SafeDispatcherServlet does not propogate exceptions generated by its underlying DispatcherServlet on
	 * init() and that it stores the exception into the ServletContext as the expected attribute name.
	 */
	@Test
	public void testInitServletConfig() {

		/*
		 * we fail if safeServlet propogates exception we rely on the underlying DispatcherServlet throwing an exception
		 * when init'ed in this way without the servlet name having been set and without there being a -servlet.xml that
		 * it can find on the classpath.
		 */
		this.safeServlet.init(this.mockConfig);

		/*
		 * here we test that the particular exception stored by the underlying DispatcherServlet has been stored into
		 * the ServetContext as an attribute as advertised by SafeDispatcherServlet. we rely on knowing the particular
		 * exception that the underlying DispatcherServlet throws under these circumstances;
		 */
	...
	/*
	 * Test that the SafeDispatcherServlet does not service requests when it has failed init and instead throws an
