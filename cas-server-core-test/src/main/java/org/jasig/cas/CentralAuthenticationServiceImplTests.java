...
public class CentralAuthenticationServiceImplTests extends AbstractCentralAuthenticationServiceTest {
	...
	public void testValidateServiceTicketWithExpires() throws Exception {
		((CentralAuthenticationServiceImpl) getCentralAuthenticationService())
				.setServiceTicketExpirationPolicy(
						new MultiTimeUseOrTimeoutExpirationPolicy(
								1,
								1100));
		final String ticketGrantingTicket = getCentralAuthenticationService()
				.createTicketGrantingTicket(
	...
	public void testValidateServiceTicketWithoutUsernameAttribute() throws Exception {
		...
		final String serviceTicket = getCentralAuthenticationService().grantServiceTicket(
				ticketGrantingTicket,
				TestUtils.getService());

		final Assertion assertion = getCentralAuthenticationService().validateServiceTicket(
				serviceTicket,
				TestUtils.getService());
	...
	public void testValidateServiceTicketWithInvalidUsernameAttribute() throws Exception {
		...
		/*
		 * The attribute specified for this service does not resolve. Therefore, we expect the default to be returned.
		 */
		assertEquals(auth.getPrincipal().getId(), cred.getUsername());
	}

	/**
	 * This test simulates : - a first authentication for a default service - a second authentication with the renew
	 * parameter and the same service (and same credentials) - a validation of the second ticket.
	 * 
	...
	/**
	 * This test checks that the TGT destruction happens properly for a remote registry. It previously failed when the
	 * deletion happens before the ticket was marked expired because an update was necessary for that.
	 *
	 * @throws AuthenticationException
	 * @throws TicketException
	 */
	...
	public void testDestroyRemoteRegistry() throws TicketException, AuthenticationException {
		...
		final TicketGrantingTicketImpl tgt = new TicketGrantingTicketImpl(
				"TGT-1",
		...
		final CentralAuthenticationServiceImpl cas = new CentralAuthenticationServiceImpl(
				...
				null,
				logoutManager);
		// destroy to mark expired and then delete : the opposite would fail with a "No ticket to update" error from the
		// registry
